#!/usr/bin/env python3
"""
Launch the SD-DarkMaster-Pro Config UI
This script properly handles both Streamlit and Gradio interfaces
"""
...
def launch_streamlit():
    ...
    print("🚀 Launching Streamlit Config UI...")
    print("="*60)
    
    # Create a streamlit-specific version of widgets
...
def launch_gradio():
    """Launch Gradio interface as fallback"""
    print("🚀 Launching Gradio Config UI (Fallback)...")
    print("="*60)
    ...
    def process_selections(model_type, selected_models, selected_loras):
        return f"Selected {len(selected_models)} models and {len(selected_loras)} LoRAs"
    
    # Create Gradio interface
    with gr.Blocks(title="SD-DarkMaster-Pro Config UI", theme="dark") as interface:
        ...
        with gr.Tabs():
            with gr.TabItem("🏠 Dashboard"):
                ...
                with gr.Row():
                    gr.Textbox(value="Workspace", label="Platform", interactive=False)
                    gr.Textbox(value="Ready", label="Status", interactive=False)
                    gr.Textbox(value="0", label="Models", interactive=False)
                    gr.Textbox(value="0 GB", label="Storage", interactive=False)
                
                with gr.Row():
                    launch_btn = gr.Button("🚀 Launch WebUI", variant="primary")
                    download_btn = gr.Button("📥 Download Models")
                    clean_btn = gr.Button("🧹 Clean Storage")
            
            with gr.TabItem("📦 Models"):
                model_type = gr.Radio(["SD 1.5", "SDXL"], label="Model Type", value="SD 1.5")
            ...
            with gr.TabItem("🎨 LoRA"):
                lora_category = gr.Dropdown(
                    ["Character", "Style", "Concept", "Pose", "Clothing", "Background"],
                    label="Category",
                    value="Character"
                )
                
                example_loras = [f"LoRA {i+1}" for i in range(10)]
            ...
            with gr.TabItem("⚙️ Settings"):
                with gr.Row():
                    ...
                    port = gr.Number(value=7860, label="Port")
                
                with gr.Row():
                    share = gr.Checkbox(label="Enable Share", value=False)
                    api = gr.Checkbox(label="Enable API", value=True)
                    xformers = gr.Checkbox(label="Enable xFormers", value=False)
        ...
        model_type.change(update_model_choices, inputs=[model_type], outputs=[selected_models])
...
def main():
    print("\n" + "="*60)
    print("🎨 SD-DarkMaster-Pro Config UI Launcher")
    print("="*60 + "\n")
    ...
    try:
        ...
        if not try_streamlit:
            print("\n❌ Error: Neither Streamlit nor Gradio is available!")
            sys.exit(1)
    
    print()
    
    # Try Streamlit first, fall back to Gradio
    if try_streamlit:
        try:
            launch_streamlit()
        except Exception as e:
            print(f"\n⚠️ Streamlit failed: {e}")
